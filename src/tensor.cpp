...
TensorHandle tensor_zeroes(MType mtype, DType dtype, size_t num_dims,
                           size_t *dims, bool learnable,
  ...
  TensorHandle result =
      (TensorHandle)arena_alloc(allocator, sizeof(Tensor), alignof(Tensor));
  result->num_dims = num_dims;
  result->dims = dims;
  result->strides = (size_t *)arena_alloc(allocator, sizeof(size_t) * num_dims,
                                          alignof(size_t));
...
TensorHandle tensor_zeroes(MType mtype, DType dtype,
                           std::initializer_list<size_t> dims, bool learnable,
                           ArenaAllocatorHandle allocator) {
  size_t *result_dims = (size_t *)arena_alloc(
      allocator, sizeof(size_t) * dims.size(), alignof(size_t));
  memcpy(result_dims, dims.begin(), sizeof(size_t) * dims.size());
  return tensor_zeroes(mtype, dtype, dims.size(), result_dims, learnable,
                       allocator);
}

// dims need to belong to the same arena as allocator
TensorHandle tensor_random(MType mtype, DType dtype, size_t num_dims,
                           size_t *dims, bool learnable,
                           ArenaAllocatorHandle allocator) {
  TensorHandle result =
...
TensorHandle tensor_random(MType mtype, DType dtype,
                           std::initializer_list<size_t> dims, bool learnable,
                           ArenaAllocatorHandle allocator) {
  size_t *result_dims = (size_t *)arena_alloc(
      allocator, sizeof(size_t) * dims.size(), alignof(size_t));
  memcpy(result_dims, dims.begin(), sizeof(size_t) * dims.size());
  return tensor_random(mtype, dtype, dims.size(), result_dims, learnable,
                       allocator);
}

TensorHandle tensor_from_vector(MType mtype, DType dtype,
                                std::initializer_list<size_t> dims,
                                bool learnable, const std::vector<float> &data,
                                ArenaAllocatorHandle allocator) {
...
TensorHandle tensor_from_data(MType mtype, DType dtype,
                              std::initializer_list<size_t> dims,
