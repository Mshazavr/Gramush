...
ArenaAllocatorHandle arena_init(size_t n_bytes, size_t cuda_n_bytes) {
  ArenaAllocatorHandle arena =
      (ArenaAllocatorHandle)malloc(sizeof(ArenaAllocator));
...
void *arena_alloc(ArenaAllocatorHandle allocator, size_t n_bytes,
                  size_t alignment) {
...
void *arena_cuda_alloc(ArenaAllocatorHandle allocator, size_t n_bytes,
                       size_t alignment) {
  uintptr_t next_num = (uintptr_t)allocator->cuda_next;
  next_num = (next_num + alignment - 1) & ~(alignment - 1);
  allocator->cuda_next = (char *)next_num;

  if (allocator->cuda_next + n_bytes >
      allocator->cuda_buffer + allocator->cuda_size) {
    // TODO: fix this
...
void arena_reset(ArenaAllocatorHandle allocator) {
  memset(allocator->buffer, 0,
         sizeof(char) * std::min(uintptr_t(allocator->next - allocator->buffer),
                                 allocator->size));
  allocator->next = allocator->buffer;

  cudaMemset(allocator->cuda_buffer, 0,
             std::min(uintptr_t(allocator->cuda_next - allocator->cuda_buffer),
                      allocator->cuda_size));
