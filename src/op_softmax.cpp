...
TensorHandle softmax(TensorHandle input, ArenaAllocatorHandle arena,
  ...
  for (size_t sub_tensor_ind = 0; sub_tensor_ind < num_sub_tensors;
    ...
    for (size_t i = 0; i < n; ++i) {
      result_data[sub_tensor_ind * n + i] =
          std::exp(input_data[sub_tensor_ind * n + i]);
  ...
  for (size_t sub_tensor_ind = 0; sub_tensor_ind < num_sub_tensors;
    ...
    for (size_t i = 0; i < n; ++i) {
      input_grads[sub_tensor_ind * n + i] +=
          (out_data[sub_tensor_ind * n + i] *
           (1.0 - out_data[sub_tensor_ind * n + i]) *
