...
int main() {
  ...
  MnistData full_data =
      load_full_mnist_dataset(train_img, train_lbl, test_img, test_lbl);
  ...
  ArenaAllocatorHandle persistent_tensors_arena =
      arena_init(ESTIM_BYTES, ESTIM_BYTES);
  ...
  for (size_t i = 0; i < num_iterations; ++i) {
    data_slices.push_back({tensor_from_data(MType::CUDA_DEVICE, DType::FLOAT32,
                                            {batch_size, input_size}, false,
                                            full_data.images_train.data() +
                                                (i * batch_size * input_size),
                                            persistent_tensors_arena),
                           tensor_from_data(MType::CUDA_DEVICE, DType::FLOAT32,
                                            {batch_size, num_labels}, false,
                                            full_data.labels_train.data() +
                                                (i * batch_size * num_labels),
                                            persistent_tensors_arena)});
  }

  // Learnable tensors
  TensorHandle w1 =
      tensor_random(MType::CUDA_DEVICE, DType::FLOAT32, {input_size, 300}, true,
                    persistent_tensors_arena);
  TensorHandle b1 = tensor_random(MType::CUDA_DEVICE, DType::FLOAT32, {300},
                                  true, persistent_tensors_arena);
  TensorHandle w2 = tensor_random(MType::CUDA_DEVICE, DType::FLOAT32,
                                  {300, 100}, true, persistent_tensors_arena);
  TensorHandle b2 = tensor_random(MType::CUDA_DEVICE, DType::FLOAT32, {100},
                                  true, persistent_tensors_arena);
  TensorHandle w3 =
      tensor_random(MType::CUDA_DEVICE, DType::FLOAT32, {100, num_labels}, true,
                    persistent_tensors_arena);
  TensorHandle b3 = tensor_random(MType::CUDA_DEVICE, DType::FLOAT32,
                                  {num_labels}, true, persistent_tensors_arena);
  ...
  for (size_t epoch_ind = 0; epoch_ind < num_epochs; ++epoch_ind) {
    ...
    for (size_t it_ind = 0; it_ind < num_iterations; ++it_ind) {
      TensorHandle l2 = addition(
          ...
          broadcast(b1, batch_size, tensors_arena, ctx), tensors_arena, ctx);
      TensorHandle z2 = relu(l2, tensors_arena, ctx);

      TensorHandle l3 = addition(mmul(z2, w2, tensors_arena, ctx),
                                 broadcast(b2, batch_size, tensors_arena, ctx),
                                 tensors_arena, ctx);
      TensorHandle z3 = relu(l3, tensors_arena, ctx);

      TensorHandle l4 = addition(mmul(z3, w3, tensors_arena, ctx),
                                 broadcast(b3, batch_size, tensors_arena, ctx),
                                 tensors_arena, ctx);

      // TensorHandle p = softmax(l4, tensors_arena, ctx);
      // TensorHandle loss = cross_entropy(p, data_slices[it_ind].second,
      // tensors_arena, ctx);
      TensorHandle loss =
          logsoftmax(l4, data_slices[it_ind].second, tensors_arena, ctx);
      ...
      std::cout << "Iteration: " << it_ind << " Loss: " << final_loss
                << std::endl;
    }
  }

  computation_context_free(ctx);
  arena_free(tensors_arena), arena_free(persistent_tensors_arena);
  return 0;
}