...
__global__ void logsoftmax(float *A, float *B, float *C, float *row_exp_sum,
                           float *row_max, int batch_size, int vec_size) {
  ...
  cuda::atomic_ref<float, cuda::thread_scope_device> batch_row_max(
      row_max[blockIdx.y]);
  if (threadIdx.x == 0) {
  ...
  // compute the exp sum for current batch index and store in
  // exp_sum[blockIdx.y]
  // ----------------------------------------------------------------------------
  ...
  cuda::atomic_ref<float, cuda::thread_scope_device> batch_row_exp_sum(
      row_exp_sum[blockIdx.y]);
  if (threadIdx.x == 0) {
...
void cuda_logsoftmax(float *A, float *B, float *C, float *row_exp_sum,
                     float *row_max, int batch_size, int vec_size) {
  dim3 gridDim(1, batch_size);
  logsoftmax<<<gridDim, BLOCK_SIZE>>>(A, B, C, row_exp_sum, row_max, batch_size,
                                      vec_size);
...
__global__ void logsoftmax_backward(float *A, float *A_grads, float *B,
                                    float *C_grads, float *row_exp_sum,
                                    float *row_max, int batch_size,
                                    int vec_size) {
  int vec_index = blockIdx.x * BLOCK_SIZE + threadIdx.x;
  int work_index =
      blockIdx.y * vec_size + blockIdx.x * BLOCK_SIZE + threadIdx.x;

  if (vec_index < vec_size) {
    float c_grad = C_grads[blockIdx.y];
    A_grads[work_index] +=
        c_grad *
        (__expf(A[work_index] - row_max[blockIdx.y]) / row_exp_sum[blockIdx.y]);
...
void cuda_logsoftmax_backward(float *A, float *A_grads, float *B,
                              float *C_grads, float *row_exp_sum,
                              float *row_max, int batch_size, int vec_size) {
  dim3 gridDim(CEIL_DIV(vec_size, BLOCK_SIZE), batch_size);
  logsoftmax_backward<<<gridDim, BLOCK_SIZE>>>(
      A, A_grads, B, C_grads, row_exp_sum, row_max, batch_size, vec_size);
  CUDA_CHECK(cudaGetLastError());
  CUDA_CHECK(cudaDeviceSynchronize());
}