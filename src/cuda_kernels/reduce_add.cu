...
__global__ void reduce_add_backward(float *A, float *B, int M, int N,
                                    float scale) {
  int vec_index = blockIdx.x * blockDim.x + threadIdx.x;
  int work_index = blockIdx.y * N + blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void reduce_add_vertical(float *A, float *B, int M, int N,
                                    float scale) {
  ...
  for (int ind = 0;
       ind < THREAD_TILE_HEIGHT && ind + blockIdx.y * THREAD_TILE_HEIGHT < M;
       ++ind) {
