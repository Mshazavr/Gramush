...
std::vector<float> read_mnist_images(const std::string &path,
  ...
  if (magic != 2051)
    throw std::runtime_error("Invalid magic number in image file: " + path);
...
std::vector<float> read_mnist_labels(const std::string &path,
                                     uint32_t expected_num_images) {
  std::ifstream file(path, std::ios::binary);
  if (!file.is_open()) {
  ...
  if (magic != 2049)
    throw std::runtime_error("Invalid magic number in label file: " + path);
  if (num_labels != expected_num_images)
    throw std::runtime_error("Label count mismatch in file: " + path);
...
MnistData load_full_mnist_dataset(const std::string &train_img_path,
  ...
  dataset.images_train = read_mnist_images(
      train_img_path, dataset.num_train_images, dataset.pixels_per_image);
  dataset.labels_train =
      read_mnist_labels(train_lbl_path, dataset.num_train_images);

  uint32_t test_pixels;
  dataset.images_test =
      read_mnist_images(test_img_path, dataset.num_test_images, test_pixels);

  if (test_pixels != dataset.pixels_per_image) {
    throw std::runtime_error(
        "Train and Test sets have different image dimensions!");
  }

  dataset.labels_test =
      read_mnist_labels(test_lbl_path, dataset.num_test_images);
...
void print_mnist_image_float(const MnistData &dataset, bool from_train_set,
  ...
  uint32_t max_index =
      from_train_set ? dataset.num_train_images : dataset.num_test_images;
  if (image_index >= max_index) {
    std::cerr << "Index out of bounds!" << std::endl;
    return;
  }

  const std::vector<float> &target_images =
      from_train_set ? dataset.images_train : dataset.images_test;
  const std::vector<float> &target_labels =
      from_train_set ? dataset.labels_train : dataset.labels_test;
