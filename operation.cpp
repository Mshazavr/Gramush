...
TensorHandle mmul(TensorHandle left, TensorHandle right, ArenaAllocatorHandle arena, ComputationContextHandle ctx) {
    ...
    TensorHandle result = tensor_zeroes(left->mtype, left->dtype, num_dims, result_dims, false, arena);
...
TensorHandle logsoftmax(TensorHandle left, TensorHandle right, ArenaAllocatorHandle arena, ComputationContextHandle ctx) {
    ...
    TensorHandle row_exp_sum = tensor_zeroes(left->mtype, left->dtype, left->num_dims - 1, row_exp_sum_dims, false, arena);
    ...
    TensorHandle row_max = tensor_zeroes(left->mtype, left->dtype, left->num_dims - 1, row_max_dims, false, arena);
...
TensorHandle mean(TensorHandle input, ArenaAllocatorHandle arena, ComputationContextHandle ctx) {
    ...
    TensorHandle result = tensor_zeroes(input->mtype, input->dtype, input->num_dims - 1, result_dims, false, arena);
...
TensorHandle broadcast(TensorHandle input, size_t n, ArenaAllocatorHandle arena, ComputationContextHandle ctx) {
    ...
    if (result->mtype == MType::HOST) {
        ...
        for (size_t i = 0; i < n; ++i) {
            if (result->dtype == DType::FLOAT32) {
                CUDA_CHECK(cudaMemcpy((float*)result->data + (i * input_size), input->data, input_size * sizeof(float), cudaMemcpyDeviceToDevice));
            } else { // result->dtype == DType::SIZE_T
                CUDA_CHECK(cudaMemcpy((size_t*)result->data + (i * input_size), input->data, input_size * sizeof(size_t), cudaMemcpyDeviceToDevice));
            }
        }
    }
...
void backward(TensorHandle tensor, ComputationContextHandle ctx) {
    ...
    if (tensor->mtype == MType::HOST) {
        ...
        float tmp = 1.0f;
        CUDA_CHECK(cudaMemcpy(tensor->grads, &tmp, dtype_sz(tensor->dtype), cudaMemcpyHostToDevice));
    }
